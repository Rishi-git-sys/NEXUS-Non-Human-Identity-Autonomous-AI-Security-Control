  ...
  return (
    <div className="overflow-x-auto w-full">
      <table className="w-full text-left text-sm text-primary-text border-collapse">
        <thead className="text-xs text-secondary uppercase border-b border-border">
          <tr>
        ...
        <tbody>
          {events.map((event) => (
            <tr key={event.id} className="border-b border-border hover:bg-surface-mid/30 transition-colors">
              <td className="px-4 py-3 font-medium">{event.actor}</td>
              <td className="px-4 py-3 font-mono text-xs">{event.action}</td>
              <td className="px-4 py-3 font-mono text-xs text-secondary">{event.resource}</td>
